...
def get_tokenizer(model_args: ModelArguments, data_args: DataArguments) -> PreTrainedTokenizer:
    ...
    if data_args.chat_template is not None:
        if os.path.exists(data_args.chat_template):
            tokenizer.chat_template = open(data_args.chat_template,'r').read()
            warnings.warn(f"chat_template is set to {data_args.chat_template}")
        else:
