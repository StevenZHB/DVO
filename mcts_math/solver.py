...
class Solver(BaseModel):
    ...
    def solve(self, solvers: List[BaseTree]):
        ...
        for step in tqdm(range(self.max_solver_steps), desc="Step Processing"):
            ...
            for out in outputs:
                for completions in out.choices:
                    if isinstance(completions.matched_stop, int):
                        completions.matched_stop = self.tokenizer.convert_ids_to_tokens(completions.matched_stop)
            ...
            # if step % 10 == 9:
            #     outputs_every_10_steps.append(self.output(solvers))

        return self.output(solvers)
        # return outputs_every_10_steps
