...
class BaseTree(BaseModel):
    ...
    ground_truth: Optional[Union[str, List[str], Dict]] = None
    ...
    @abstractmethod
    def init_code_solution(self) -> None:
        """
        Init the golden solution to the tree.
        """
        pass
    ...
    def __repr__(self):
        return (
            ...
            f"llm_model_id={self.llm_model_id}, "
            f"unique_id={self.unique_id!r}, "
            f"root_tag={self.root.tag if self.root else 'None'}, "
            f"current_node_tag={self.current_node.tag if self.current_node else 'None'})"
        )
