...
from mcts_math.math_evaluation import is_equiv_no_timeout, is_equiv
...
def math_is_equiv_no_timeout(grt: Union[str, list[str]], prd: str):
    prd = remove_single_dollar(prd)
    if isinstance(grt, list):
        for g in grt:
            if is_equiv_no_timeout(remove_single_dollar(g), prd):
                return True
        return False
    else:
        return is_equiv_no_timeout(remove_single_dollar(grt), prd)
