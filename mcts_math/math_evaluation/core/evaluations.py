        ...
        if isinstance(ground_truth_normalized, set) or isinstance(given_normalized, set):
            ...
            for gt_norm in ground_truth_normalized:
                for gv_norm in given_normalized:
                    if is_latex_equiv_no_timeout(gt_norm, gv_norm, verbose=verbose):
                        return True
            return False
        else:
            return is_latex_equiv_no_timeout(ground_truth_normalized, given_normalized, verbose=verbose)

    except Exception as e:
        return False 

def is_equiv_no_timeout(ground_truth: str, given: str, verbose: bool = False):
    if ground_truth is None and given is None:
        print("WARNING: Both None")
        return True
    if ground_truth is None or given is None:
...
def is_latex_equiv_no_timeout(
    ...
    if not is_correct and not splitted:
        is_correct = are_equal_under_sympy_no_timeout(ground_truth_normalized, given_normalized, verbose)
