...
class Solver(BaseModel):
    ...
    def generate_postprocess(
        ...
        with ProcessPool(max_workers=min(len(valid_solvers), os.cpu_count()//8-1)) as pool:
    ...
    def solve(self, solvers: List[BaseTree]):
        ...
        for step in tqdm(range(self.max_solver_steps), desc="Step Processing"):
            ...
            outputs = self.llm(prompts, self.generate_sampling_params,use_tqdm=True if self.config.verbose else False)
            ...
            if self.need_value_func:
                outputs = self.llm(prompts, self.value_sampling_params,use_tqdm=True if self.config.verbose else False)
                reconstructed_outputs = [outputs[bos_idx : eos_idx] for bos_idx, eos_idx in zip(prompts_span, prompts_span[1:])]
            else:
